<!-- repository: services/event | kind: SERVICE | stack: nestjs -->

# event — Skill: Service Development

> Workflow for event (services/event). Execute this workflow before, during, and
...
- Description: Omnixys Event Service – event domain (events, layout, catalog, admin, analytics).
- Architecture: src/adapter, admin, analytics, config, core, event, handlers, health, prisma, security
...
- Tests: node --test __tests__/unit/*.test.mjs; Jest e2e (infrastructure-dependent)
...
- Identify the affected bounded context within `src/adapter, admin, analytics, config, core, event, handlers, health, prisma, security`.
...
- Keep tenant isolation intact (`Multi-tenant: event data must carry tenant predicates.`).
