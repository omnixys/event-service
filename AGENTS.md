...
> Repository development instructions for **event** (Service).
...
src/adapter, admin, analytics, config, core, event, handlers, health, prisma, security
...
@omnixys/contracts-ts, cache-ts, context-ts, graphql-ts, kafka-ts, logger-ts, observability-ts, security-ts
...
node --test __tests__/unit/*.test.mjs; Jest e2e (infrastructure-dependent)

## Repository-Specific Rules

Multi-tenant: event data must carry tenant predicates.
